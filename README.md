...
- **内置工具**(`src/tools.ts`): `read` / `write` / `edit` / `glob` / `grep` / `bash` / `http_get` / `http_post` / `llm`
  - 读类( read / glob / grep / http_get / http_post )默认放行;写类(write / edit)与命令(bash)触发确认
  - 输出截断( read 50KB / bash 4000 字符 / grep 200 条 )
- **界面**: 交互式 Ink TUI(输入框、流式文本渲染、工具调用状态、权限确认、Esc 中断),以及 headless 模式(管道/stdin 单轮)
...
`pnpm typecheck && pnpm smoke && pnpm smoke:influx` 必须全绿。smoke 覆盖: 工具回环、回喂、doom-loop、坏参 JSON、权限拒绝;influx smoke 覆盖: 干跑预览、缓存命中、when 分支、错误阻断、fallback、retry、自动 key、重复 key 快速失败。

## 未实现(明确不做/后续)

- 上下文压缩与历史截断: 目前整段回放消息(先 brutal 截断)
- 会话持久化、多轮跨进程恢复
- 子 agent / MCP client / 插件
- 并发工具调用(当前顺序执行,代码已预留)
- 图片输入、多 provider 协议(non-OpenAI)
- 计划运行时与对话 agent 的深度统一(当前共用 CLI/环境变量/部分工具,两套循环各自独立)

> 诚实说明: 真实 LLM 端到端未经当前环境验证(无 LLM_URL 配置)。smoke 用脚本化假 server 验证了协议与循环逻辑;接入真实模型需按上文配置环境后人工验证。
